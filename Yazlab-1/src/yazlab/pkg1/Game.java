...
class StartGame extends Thread {
    ...
    public int[] PositionD =  {(int) N,(int) M};
    ...
    public StartGame(Integer[][] EditedGold, Integer[][] EditedSGold, ArrayList<JButton> GameArea, JLabel ScoreBoardA, JLabel ScoreBoardB, JLabel ScoreBoardC, JLabel ScoreBoardD, int DeftMovement, int DeftMovementPrice, int ANTPrice, int BNTPrice, int CNTPRice, int DNTPrice, int CSuperMode, int DGold, float M, float N) throws IOException {
        ...
        this.PositionB[0] = 1;
        this.PositionB[1] = (int) this.N;
        
        this.PositionC[0] = (int) this.M;
        this.PositionC[1] = 1;
        
        this.PositionD[0] = (int) this.M;
        this.PositionD[1] = (int) this.N;
        ...
        if (Mode == 0) {
            for (int i = 0; i < EditedSGold.length; i++) {
                try{
                    ...
                    System.out.println("Error: "+i+" "+"EditSGoldLength: " + EditedSGold.length+" EditedSGold[i][0]: "+EditedSGold[i][0] +" EditedSGold[i][1]: "+EditedSGold[i][1] );
                }
            }
        } else if (Mode == 1) {
            boolean status;
    ...
    public int[] Movement(int[] Position, int[] NewPosition, Icon character, int Mode, File player) {
        System.out.println(Position[0] + " " + Position[1]);
        boolean IsSercetGold = false;
        int[] OldPosition = new int[2];
        int MC = 0;

        while (Position[0] != NewPosition[0] && MC < DefaultMovement) {
        ...
        while (Position[1] != NewPosition[1] && MC < DefaultMovement) {
            ...
            if (WayPredictNGA_D < WayPredictNGA_A) {
                ...
                for (int i = 0; i < EditedGold.length; i++) {
                    if (EditedGold[i][0] == PredictedNGA[0] && EditedGold[i][1] == PredictedNGA[1]) {
                        resultAD = (float) WayPredictNGA_D * PropertyD[0];
                        break;
                    }
                }
            ...
            if (WayPredictNGA_D < WayPredictNGB_B) {
                ...
                for (int i = 0; i < EditedGold.length; i++) {
                    if (EditedGold[i][0] == PredictedNGB[0] && EditedGold[i][1] == PredictedNGB[1]) {
                        resultBD = (float) WayPredictNGB_D * PropertyD[0];
                        break;
                    }
                }
                ...
                for (int i = 0; i < EditedGold.length; i++) {
                    if (EditedGold[i][0] != Position[0] || EditedGold[i][1] != Position[1]) {
                        if (EditedGold[i][0] == PredictedNGC[0] && EditedGold[i][1] == PredictedNGC[1]) {
                            resultCD = (float) WayPredictNGC_D  * PropertyD[0];
                            break;
                        }
                    }
                }
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (HamleSirasi == 1) {
                ...
                int[] Position = Movement(PositionA, BestGoldA, charachterA, 0, playerA);
                if(BestGoldA[0] == Position[0] && BestGoldA[1] == Position[1]){
                    PositionA = Position;
                    for (int i = 0; i < EditedGold.length; i++) {
                ...
                //System.out.println(PositionA[0]+"-"+PositionA[1]+"/"+NearestGold[0]+"-"+NearestGold[1]);
                int[] Position = Movement(PositionB, BestGoldB, charachterB, 0, playerB);
                if(BestGoldB[0] == Position[0] && BestGoldB[1] == Position[1]){
                    PositionB = BestGoldB;
                    for (int i = 0; i < EditedGold.length; i++) {
                ...
                //System.out.println(PositionA[0]+"-"+PositionA[1]+"/"+NearestGold[0]+"-"+NearestGold[1]);
                int[] Position = Movement(PositionC, BestGoldC, charachterC, 1, playerC);
                if(BestGoldC[0] == Position[0] && BestGoldC[1] == Position[1]){
                    PositionC = BestGoldC;
                    for (int i = 0; i < EditedGold.length; i++) {
                ...
                //System.out.println(PositionA[0]+"-"+PositionA[1]+"/"+NearestGold[0]+"-"+NearestGold[1]);
                int[] Position = Movement(PositionD, BestGoldD, charachterD, 0, playerD);
                if(BestGoldD[0] == Position[0] && BestGoldD[1] == Position[1]){
                    PositionD = BestGoldD;
                    for (int i = 0; i < EditedGold.length; i++) {
...
public class Game extends Thread {
    ...
    public static void Start() {
        ...
        while (bulunan != (int) (M * N / 100 * AltinOrani)) {
